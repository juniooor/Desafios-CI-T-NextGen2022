'''Uma famosa empresa de streams de jogos quer fornecer para seus streamers um mecanismo para que eles possam 
realizar sorteios nas suas lives de formas a dar mais chances para seus fãs mais fiéis. Você foi contratado com
a missão de ajudar a automatizar esse sorteio, sendo que ele deve seguir as seguintes regras:
A chance de uma pessoa ser sorteada é diretamente proporcional às horas de conteúdo que ela assistiu naquele canal. 
Assim sendo, uma pessoa que ficou 20 horas acompanhando esse streamer tem o dobro de chances de ser sorteado do que
uma pessoa que o assistiu durante 10 horas, se ambas não forem assinantes pagos. O número de horas é recebido em minutos
e é sempre arredondado para cima, ou seja:
181 minutos = 4 horas
239 min = 4h
180 min = 3h
...
def calcula_porcentagem_sorteio(assinante,minutos_assistidos):
    porcentagem = []
    times = []
    horas_assistidos = list(map(lambda x: math.ceil(x/60), minutos_assistidos))
    for user,time in zip(assinante,horas_assistidos):
        if user == True:
            time *= 2
            horas_assistidos[assinante.index(user)] = time
            times.append(time)
        else:
            times.append(time)
    porcentagem = list(map(lambda x: int(round((x/sum(horas_assistidos)) * 100)), times ))
    print(sum(horas_assistidos))
    return porcentagem
...
if __name__ == '__main__':
    # a = calcula_porcentagem_sorteio([True,False],[60,120]) #6
    a = calcula_porcentagem_sorteio([True,False,False,False,True,False,True,True,False,False,False],[6144,2742,330,30,1500,4032,24036,3288,2076,24540,4716]) #3
    print(a)